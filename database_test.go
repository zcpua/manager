...
func TestConcatDSN_String_null(t *testing.T) {
	var setting []Setting
	ass := assert.New(t)
	setting = append(setting, SetCollation(""),SetLoc("Asia/Shanghai"), SetTimeZone("Asia/Shanghai"), SetStrict(false), SetInterpolateParams(true))
	ass.Equal("loc=Asia%2FShanghai&time_zone=%27Asia%2FShanghai%27&strict=false&interpolateParams=true", concatDSN(setting), `null value should be ignored`)
}

func TestConcatDSN_TimeZone(t *testing.T) {
	var setting []Setting
	ass := assert.New(t)
	setting = append(setting, SetLoc("Asia/Shanghai"), SetTimeZone("Asia/Shanghai"))
	ass.Equal("loc=Asia%2FShanghai&time_zone=%27Asia%2FShanghai%27", concatDSN(setting), `null value should be ignored`)
}
